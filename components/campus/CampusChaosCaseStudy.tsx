...
import { ArrowLeft, Banknote, BookOpen, Coffee, Copy, Gauge, GraduationCap, Lock, Play, Plus, Sparkles, Timer, Trophy, Zap } from 'lucide-react'
...
const loopSteps = [
  ...
  'Students create traffic and demand',
  'Events add urgency and chaos',
  'The player upgrades key facilities',
  'Reputation unlocks better campus tools',
]

const systems: Array<{ title: string; body: string; Icon: IconType }> = [
  { title: 'Core gameplay loop', body: 'Collect income, react to campus events, upgrade buildings and keep the university productive.', Icon: Play },
  { title: 'Economy system', body: 'Campus Cash funds upgrades, Chaos Points measure pressure, and Reputation unlocks progress.', Icon: Banknote },
  { title: 'Building system', body: 'Canteen starts the loop, Copy Center creates risk, and Library becomes a reputation-focused unlock.', Icon: GraduationCap },
  { title: 'Event system', body: 'Short popups like Printer Jam create quick interactive moments inside the idle loop.', Icon: Zap },
  { title: 'AFK progression', body: 'A compact report shows rewards earned while away from the game.', Icon: Timer },
  { title: 'Interface clarity', body: 'The prototype focuses on mobile readability, visual hierarchy and responsive interaction.', Icon: Gauge },
]

const visualPillars = [
  'Vertical 9:16 mobile-first layout',
  'Premium portfolio framing',
  'Illustrated campus building cards',
  'Bright game UI without childish noise',
  'Readable hierarchy and soft motion',
]
...
export default function CampusChaosCaseStudy() {
  ...
  const [buildings, setBuildings] = useState<Building[]>([
    { id: 'canteen', name: 'Canteen', status: 'Live', level: 3, income: 420, description: 'The starting building: fast income, student foot traffic and clear early-game upgrade readability.', image: '/campus-chaos/building-canteen.svg', icon: <Coffee className="h-5 w-5" />, color: 'from-amber-300 to-orange-400' },
  ...
  return (
    <main className="mx-auto w-[min(1240px,calc(100%-28px))] pb-16 pt-28 text-white">
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-[.95fr_1.05fr]">
        <CasePanel eyebrow="01 / Project overview" title="A premium mobile game concept presentation">
          <p className="text-white/68 leading-8">The project is presented as a focused game design and UI prototype: readable systems, clean mobile layout, custom campus visuals and enough interaction to make the concept feel real inside a portfolio.</p>
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            <Metric label="Income / min" value={`$${incomePerMinute}`} />
            <Metric label="Prototype scope" value="Web UI" />
            <Metric label="Format" value="9:16 mobile" />
          </div>
        </CasePanel>

        <CasePanel eyebrow="02 / Core gameplay loop" title="Simple loop, clear game value">
          <div className="grid gap-3">
            {loopSteps.map((step, index) => (
              <motion.div key={step} initial={{ opacity: 0, x: 14 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[.045] p-4">
                <span className="grid h-9 w-9 place-items-center rounded-xl bg-cyan-300 text-sm font-black text-slate-950">{index + 1}</span>
                <span className="font-bold text-white/78">{step}</span>
              </motion.div>
            ))}
          </div>
        </CasePanel>
      ...
      <section className="mt-8 rounded-[2rem] border border-cyan-300/15 bg-slate-950/70 p-6 md:p-8">
        <div className="mb-7 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.32em] text-cyan-200/70">03 / Main systems</p>
            <h2 className="mt-2 font-display text-4xl font-black tracking-[-0.05em] md:text-6xl">Design systems</h2>
          </div>
          <p className="max-w-xl text-white/60">Economy, progression, event rhythm and interface clarity.</p>
        </div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {systems.map(({ title, body, Icon }) => (
            <motion.article key={title} whileHover={{ y: -8, rotate: 0.35 }} className="rounded-3xl border border-white/10 bg-white/[.045] p-6 shadow-xl shadow-black/20">
      ...
      <section className="mt-8 grid gap-8 lg:grid-cols-[420px_1fr] lg:items-start" id="prototype">
        <div className="sticky top-28">
          <p className="text-xs font-black uppercase tracking-[0.32em] text-amber-100/80">04 / Interactive prototype</p>
          <h2 className="mt-2 font-display text-5xl font-black leading-none tracking-[-0.06em] md:text-7xl">Phone mockup with proper visuals</h2>
          <p className="mt-5 leading-8 text-white/64">Illustrated buildings, animated student avatars, event interaction, upgrade buttons and AFK reward modal are shown inside a vertical mobile game mockup.</p>
          <div className="mt-6 grid gap-3">
            {visualPillars.map((item) => <div key={item} className="rounded-2xl border border-white/10 bg-white/[.045] px-4 py-3 text-sm font-bold text-white/72">{item}</div>)}
          </div>
        </div>

        <div className="grid gap-6 xl:grid-cols-[390px_1fr]">
          <PhonePrototype cash={cash} chaos={chaos} reputation={reputation} buildings={buildings} rewardVisible={rewardVisible} eventVisible={eventVisible} onResolveEvent={resolveEvent} onUpgrade={upgradeBuilding} onOpenAfk={() => setAfkOpen(true)} />
...
function Hero() {
  return (
    <section className="relative overflow-hidden rounded-[2.4rem] border border-cyan-300/20 bg-slate-950/80 p-6 shadow-2xl shadow-black/40 md:p-10">
      ...
      <div className="relative z-10 grid gap-10 lg:grid-cols-[.92fr_1.08fr] lg:items-center">
        <div>
          <Link href="/" className="mb-6 inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-xs font-black uppercase tracking-[0.28em] text-cyan-100 hover:border-cyan-200/50"><ArrowLeft className="h-4 w-4" /> Portfolio</Link>
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-amber-200/25 bg-amber-300/10 px-4 py-2 text-xs font-black uppercase tracking-[0.28em] text-amber-100"><Sparkles className="h-4 w-4" /> Interactive mobile game case study</div>
          <h1 className="font-display text-6xl font-black leading-[.86] tracking-[-0.07em] md:text-8xl">Campus Chaos<br />Tycoon</h1>
          <p className="mt-6 max-w-2xl border-l-4 border-cyan-300/50 pl-5 text-lg leading-8 text-white/72">A polished portfolio concept for a vertical idle tycoon game where the player manages a chaotic university campus.</p>
          <div className="mt-7 flex flex-wrap gap-3">{['React / Next.js', 'Tailwind CSS', 'Framer Motion', 'Mobile Game UX', 'Idle Tycoon Concept'].map((tag) => <span key={tag} className="rounded-2xl border border-cyan-200/15 bg-white/[.045] px-4 py-3 text-sm font-bold text-white/72">{tag}</span>)}</div>
        </div>
        <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="rounded-[2rem] border border-white/10 bg-white/[.04] p-3 shadow-2xl shadow-cyan-950/40">
  ...
  return (
    <div className="grid gap-4">
      {buildings.map((building) => (
        <article key={building.id} className="overflow-hidden rounded-3xl border border-white/10 bg-white/[.045] p-5">
          <div className="grid gap-5 md:grid-cols-[170px_1fr] md:items-center">
            <div className="rounded-[1.6rem] border border-white/12 bg-black/20 p-3"><img src={building.image} alt={`${building.name} illustration`} className="h-auto w-full rounded-2xl" /></div>
            <div>
              <div className="flex items-start justify-between gap-4">
                <div className="flex gap-4">
                  ...
                  <div><h3 className="font-display text-2xl font-black">{building.name}</h3><p className="mt-1 text-sm font-bold text-white/48">{building.status} · Level {building.level} · +${building.income}/min</p></div>
                </div>
                {building.status === 'Locked' ? <Lock className="h-5 w-5 text-white/36" /> : null}
              </div>
    ...
    <div className="mx-auto w-full max-w-[390px] rounded-[3rem] border-[10px] border-black bg-black p-2 shadow-2xl shadow-cyan-950/60">
      <div className="relative min-h-[780px] overflow-hidden rounded-[2.35rem] bg-gradient-to-b from-sky-200 via-emerald-100 to-emerald-300 text-slate-950">
        ...
        <div className="relative z-10 px-4 pb-4 pt-12">
          <div className="rounded-3xl border border-white/50 bg-white/78 p-4 shadow-xl backdrop-blur"><p className="text-xs font-black uppercase tracking-[0.24em] text-emerald-700">Campus Chaos Tycoon</p><div className="mt-3 grid grid-cols-3 gap-2 text-center"><Currency label="Cash" value={`$${cash.toLocaleString()}`} /><Currency label="Chaos" value={chaos.toString()} /><Currency label="Rep" value={`${reputation}%`} /></div></div>
          <div className="relative mt-4 h-[405px] overflow-hidden rounded-[2rem] border-4 border-white/50 bg-[linear-gradient(180deg,#b3ecff_0%,#9ceba9_42%,#7fd676_100%)] shadow-inner">
            <div className="absolute left-[50%] top-0 h-full w-[70px] -translate-x-1/2 bg-[#efc988] shadow-inner" /><div className="absolute left-0 top-[48%] h-[70px] w-full -translate-y-1/2 bg-[#efc988] shadow-inner" /><div className="absolute left-[47.6%] top-[44.8%] z-[1] h-[90px] w-[90px] -translate-x-1/2 -translate-y-1/2 rounded-full border-[12px] border-[#fee8ae] bg-emerald-400/90 shadow-lg" /><div className="absolute left-[47.6%] top-[44.8%] z-[1] h-[38px] w-[38px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-emerald-600" />
            <CampusMapBuildings />
            <Students />
            <EventPopup visible={eventVisible} onResolve={onResolveEvent} />
            <Reward visible={rewardVisible} />
          </div>
          <div className="mt-4 grid gap-3">{buildings.map((building) => <div key={building.id} className="rounded-3xl border border-white/50 bg-white/82 p-3 shadow-lg backdrop-blur"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><img src={building.image} alt={`${building.name} thumb`} className="h-12 w-14 rounded-2xl object-cover" /><div><b>{building.name}</b><p className="text-xs font-bold text-slate-500">Lv. {building.level} · +${building.income}/min</p></div></div><button disabled={building.status === 'Locked'} onClick={() => onUpgrade(building.id)} className="rounded-2xl bg-slate-950 px-3 py-2 text-xs font-black text-white disabled:bg-slate-400"><Plus className="inline h-3 w-3" /> Upgrade</button></div></div>)}</div>
